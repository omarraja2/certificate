...
interface CertProps {
  name: string; course: string; level: string
  issueDate: string; certId: string; flash: boolean
}
    ...
    <div className={`cert-wrap${flash ? ' flash' : ''}`}>
      <div id="certificate">
        ...
        <div className="cert-inner">
          ...
          <div className={`cert-name${!name ? ' placeholder' : ''}`}>{name || 'Student Name Here'}</div>
          ...
          <div className="cert-footer">English Future · Certificate ID: {certId}</div>
        </div>
      </div>
    </div>
  )
...
const COURSES = [
  'Advanced English Communication', 'Business English Mastery',
  'Academic English Writing', 'Conversational English',
  'IELTS Preparation Course', 'English for Professionals', 'English from Zero',
]
...
export default function CertificatePage() {
  ...
  async function buildPDF() {
    ...
    const canvas = await html2canvas(certEl, { scale: 4, useCORS: true, backgroundColor: '#ffffff', logging: false })
    ...
    pdf.addImage(imgData, 'JPEG', 10, (H - certH) / 2, certW, certH)
    return pdf
  }
  ...
  async function generatePDF() {
    ...
    } catch (e) { alert('Error generating PDF: ' + (e as Error).message) }
    setLoading(false)
  }
  ...
  async function printPDF() {
    ...
    } catch (e) { alert('Error printing PDF: ' + (e as Error).message) }
    setPrinting(false)
  }
    ...
    <>
      ...
      <div className="page-wrap">
        <div className="form-panel">
          ...
          <p className="subtitle">Fill in the details to generate a personalized English Future certificate and download it instantly as a PDF.</p>

          <div className="field-group">
            <label>Student Full Name</label>
            <input type="text" placeholder="e.g. Anna Petrova" value={name} onChange={e => setName(e.target.value)} autoComplete="off" />
          </div>
          <div className="row-2">
            <div className="field-group">
              <label>Course</label>
          ...
          <div className="row-2">
            <div className="field-group">
              ...
              <input type="text" value="Mohammed Salim" readOnly style={{ opacity: 0.45, cursor: 'not-allowed' }} />
            </div>
            <div className="field-group">
          ...
          <div className="field-group">
            ...
            <input type="text" value="English Future" readOnly style={{ opacity: 0.45, cursor: 'not-allowed' }} />
          </div>

          <div className="btn-row">
            <button className="btn-generate" onClick={generatePDF} disabled={loading}>
              {loading ? <div className="loader" /> : <span>⬇ &nbsp; Download PDF</span>}
            </button>
        ...
        <div className="preview-panel">
          ...
          <Certificate name={name} course={course} level={level} issueDate={issueDate} certId={certId} flash={flash} />
